...
export type Room={hostId:string;phase:Phase;createdAt:number;settings:{revealSeconds:number;category:string};players:Record<string,Player>};
...
export async function createRoom(code:string,hostId:string,hostName:string,category:string,revealSeconds:number){
  const player:Player={id:hostId,name:hostName,submitted:false,leaderId:hostId,members:[],eliminated:false};
  await update(ref(db!),{[`rooms/${code}/hostId`]:hostId,[`rooms/${code}/phase`]:'lobby',[`rooms/${code}/createdAt`]:Date.now(),[`rooms/${code}/settings`]:{category,revealSeconds},[`rooms/${code}/players/${hostId}`]:player});
}
...
export async function beginReveal(code:string,hostId:string){
  ...
  const endsAt=Date.now()+(room.settings?.revealSeconds||30)*1000;
...
export async function savePlayers(code:string,hostId:string,players:Record<string,Player>){
  const snap=await get(roomRef(code));
  if(snap.val()?.hostId!==hostId)throw new Error('Only the host can update the board');
  await set(ref(db!,`rooms/${code}/players`),players);
}
