...
import './styles.css';
import {configured,login,createRoom,joinRoom,submitSecret,watchRoom,watchReveal,watchSubmissions,beginReveal,hideReveal,savePlayers,saveTheme,leaveRoom,removePlayer,type Room,type Reveal} from './firebase';
...
function App(){
  ...
  useEffect(()=>{
    ...
    if((reveal&&remainingMs(reveal.endsAt,now)===0)||(!reveal&&revealSettled))hideReveal(code,uid).catch(()=>{});
  },[now,room?.phase,reveal,revealSettled,isHost]);
  useEffect(()=>{
    if(demo&&room?.phase==='reveal'&&reveal&&remainingMs(reveal.endsAt,now)===0){setRoom(r=>r?{...r,phase:'playing'}:r);setReveal(null)}
  ...
  async function host(){
    ...
    if(demo){
      ...
      setRoom({hostId:uid,phase:'lobby',createdAt:Date.now(),settings:{category,revealSeconds:duration},players:{[uid]:me,...extra}});
      saveRoom(c);setScreen('game');
    }else{
      try{await createRoom(c,uid,name,category,duration);saveRoom(c);setScreen('game')}catch(e){fail(e)}
    }
  }
  async function join(){
  ...
  async function leave(){
    try{if(!demo&&room?.players?.[uid])await leaveRoom(code,uid)}catch{}
  ...
  async function start(){
    ...
    try{
      if(demo){
        ...
        setRoom({...room,phase:'reveal'});setReveal({words,endsAt:Date.now()+duration*1000});
  ...
  async function doCapture(a:string,b:string){
    ...
    try{
      ...
      if(demo)setRoom({...room,players,phase:winner(players)?'finished':'playing'});
      else await savePlayers(code,uid,players);
    }catch(e){fail(e)}
  }
  ...
  if(screen==='setup')return <Page {...pageProps}><Card title="Host a game" back={()=>setScreen('home')}><Field label="Your name" value={name} set={setName} placeholder="Param"/><Field label="Secret category" value={category} set={setCategory} placeholder="Cities"/><label>Reveal time <b>{duration} seconds</b><input type="range" min="15" max="60" step="5" value={duration} onChange={e=>setDuration(+e.target.value)}/></label><button disabled={!uid} onClick={host}>Create room</button><ErrorText text={error}/></Card></Page>;
  ...
  if(room.phase==='reveal'){
    if(isHost&&reveal){
      ...
      return <Page {...pageProps}><section className="reveal"><div className="timer">{left}</div><p>Only you can see this list</p><div className="word-grid">{reveal.words.map(w=><span key={w}>{w}</span>)}</div><button className="ghost" onClick={endReveal}>Hide now</button></section></Page>;
    }
  ...
  if(room.phase==='playing'||room.phase==='finished')return <Page {...pageProps}>
    ...
    <Board room={room} host={isHost} capture={doCapture} code={code} submissions={submissions} onRevealAgain={start}/>
...
function Board({room,host,capture,code,submissions,onRevealAgain}:{room:Room;host:boolean;capture:(a:string,b:string)=>void;code:string;submissions:Record<string,{word:string}>;onRevealAgain:()=>void}){
  const alive=Object.values(room.players).filter(p=>p&&p.id&&p.name&&!p.eliminated);
  const win=winner(room.players);
  ...
  return <section className="board">
    ...
    {host&&<details className="identity-key"><summary>Host identity key</summary>{Object.entries(submissions).map(([id,s])=><div key={id}><span>{s.word}</span><b>{room.players[id]?.name||'Player left'}</b></div>)}</details>}
    <div className="empires">{alive.map(p=><article key={p.id}><i>{p.name[0]}</i><h3>{p.name}</h3><p>{(p.members||[]).length+1} in empire</p><div>{(p.members||[]).map(id=><span key={id}>{room.players[id]?.name}</span>)}</div></article>)}</div>
    {host&&!win&&<div className="capture">
  ...
  return <main>
    ...
    {helpOpen&&<div className="help-backdrop" role="dialog" aria-modal="true" aria-label="How to play"><section className="help-panel">
      ...
      <ol>
        ...
        <li><b>Reveal.</b> The host sees the full list for a limited time, and can bring it back later.</li>
